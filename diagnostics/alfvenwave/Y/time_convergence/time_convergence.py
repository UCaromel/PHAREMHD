...
for integrator_name in time_integrators:
    for stepDt in quantities[integrator_name][studied_quantity]:
        ...
        y = np.arange(ny)*Dy
        expected_value = 1e-6 * np.cos(2 * np.pi * (y - times[stepDt][-1] * 0.00005 / factor))
...
expected_value = 1e-6 * np.cos(2 * np.pi * (y - times[0][-1] * 0.00005))
for integrator_name in time_integrators:
    for stepDt in quantities[integrator_name][studied_quantity]:
        ...
        plt.plot(y, computed_value, label = f"dt = {Dt_values[stepDt]}")
    plt.plot(y, expected_value, label = 'expected')
    plt.legend()
    plt.show()
