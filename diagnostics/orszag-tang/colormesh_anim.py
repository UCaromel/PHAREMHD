...
fig, ax = plt.subplots()
im = ax.pcolormesh(reshaped_data[0][:, :, 7].T, cmap='coolwarm')
ax.set_aspect('equal')
fig.colorbar(im, ax=ax)

def update(frame):
    im.set_array(reshaped_data[frame][:, :, 7].T)
    plt.title(f'TotalEnergy at t={times[frame]}')
    return im,
